...
// @updateUrl    https://raw.gitmirror.com/baiyu-yu/plug-in/refs/heads/main/%E6%89%93%E4%BC%81%E9%B9%85.js
...
if (!seal.ext.find("penguinBattle")) {
  ...
  // 数据库键定义
  const dbKeyPlayerData = "penguinBattle_playerData"; // 玩家数据
  const dbKeyEncounters = "penguinBattle_encounters"; // 奇遇数据
  const dbKeyShop = "penguinBattle_shop"; // 商店数据
  const dbKeyGroupBoss = `penguinBattle_boss`; // 群组BOSS数据
  const dbKeyBossCooldown = "penguinBattle_bossCooldown"; // BOSS冷却数据
  const dbKeyPenguin = "penguinBattle_penguin"; // 小企鹅数据
  const dbKeyPenguinEgg = "penguinBattle_penguinEgg"; // 企鹅蛋数据
  const dbKeyPvP = "penguinBattle_pvp"; // PvP数据
  const dbKeyHouses = "penguinBattle_houses"; // 房产数据
  const dbKeyTitles = "penguinBattle_titles"; // 称号数据
  const dbKeySignIn = "penguinBattle_signIn"; // 签到数据
    ...
    return (player) => {
      effects.forEach((effect) => {
        ...
        if (attribute && value && !isNaN(value)) {
          player[attribute] = Math.max(
            0,
            (player[attribute] || 0) + parseInt(value, 10)
          );
        }
      });
    };
  ...
  const defaultEncounters = [
    {
      description: "你在冰洞中发现了一个神秘宝箱，获得攻击力+5",
      effectId: "attack+5",
    },
    {
      description: "遇到了一群善良的企鹅修女，为你祈祷，生命值+30",
      effectId: "maxHealth+30",
    },
    ...
    {
      description: "你发现了一本古老的战斗秘籍，攻击力+10",
      effectId: "attack+10",
    },
    { description: "你遇到了一位神秘的商人，金币+100", effectId: "gold+100" },
    {
      description: "你被一只巨大的企鹅追赶，生命值-20，但获得了速度提升",
      effectId: "13",
    },
    ...
    {
      description: "你遇到了一只受伤的小企鹅，生命值+50",
      effectId: "health+50",
    },
    {
      description: "发现隐藏的洞穴，攻击力+5 and 魔力+10",
      effectId: "attack+5&mana+10",
    },
  ...
  const defaultShop = [
    ...
    },
    {
      ...
      description: "生命值+50",
      cost: 120,
      effectId: "health+50&maxHealth+50",
    },
  ...
  const skills = [
    {
      name: "普通攻击",
      description: "对敌人造成基础攻击力伤害",
      manaCost: 0,
      effectId: "1",
    },
    {
      name: "强力一击",
      description: "对敌人造成2倍基础攻击力伤害，消耗 10 魔力",
      manaCost: 10,
      effectId: "2",
    },
    {
      name: "防御姿态",
      description: "减少50%的伤害，持续3回合，消耗 8 魔力",
      manaCost: 8,
      effectId: "3",
    },
    {
      name: "虚弱诅咒",
      description: "使敌人下回合攻击力减半，持续1回合，消耗 12 魔力",
      manaCost: 12,
      effectId: "4",
    },
    {
      name: "生命吸取",
      description: "对敌人造成伤害并恢复等量生命值，消耗 15 魔力",
      manaCost: 15,
      effectId: "5",
    },
    {
      name: "火焰风暴",
      description: "对敌人造成基础攻击力2.5倍伤害，消耗 20 魔力",
      manaCost: 20,
      effectId: "6",
    },
    {
      name: "冰冻陷阱",
      description: "使敌人下回合无法行动，持续2回合，消耗 18 魔力",
      manaCost: 18,
      effectId: "7",
    },
    {
      name: "雷霆一击",
      description: "造成3倍伤害，消耗 25 魔力",
      manaCost: 25,
      effectId: "8",
    },
    {
      name: "复活术",
      description: "复活并恢复50%生命值，消耗 50 魔力",
      manaCost: 50,
      effectId: "9",
    },
    {
      name: "愈合术",
      description: "恢复自身生命值15点，消耗 10 魔力",
      manaCost: 10,
      effectId: "14",
    },
    {
      name: "护盾",
      description: "为自身添加一个抵挡30点伤害的护盾，持续3回合，消耗 25 魔力",
      manaCost: 25,
      effectId: "15",
    },
    {
      name: "毒刃",
      description:
        "对敌人造成攻击力的伤害，并在接下来的2回合每回合造成5点额外伤害",
      manaCost: 20,
      effectId: "16",
    },
  ];
  ...
  const onBossDeath = (groupId, boss) => {
    ...
    participants.forEach((playerId) => {
      ...
      seal.replyToSender(
        ctx,
        msg,
        `${player.name} 获得了 ${rewardGold} 金币和 ${rewardExp} 经验！`
      );
      ...
      if (player.level % 10 === 0) {
        ...
        if (randomStatIncrease < 0.5) {
          ...
          seal.replyToSender(
            ctx,
            msg,
            `${player.name} 的 ${statToIncrease} 增加了！`
          );
        } else {
          // 随机获得技能（从技能表中选择）
          const availableSkills = skills.filter(
            (skill) =>
              !player.skills.some(
                (playerSkill) => playerSkill.name === skill.name
              )
          );
          if (availableSkills.length > 0) {
            ...
            seal.replyToSender(
              ctx,
              msg,
              `${player.name} 获得了新技能: ${newSkill.name}!`
            );
          } else {
            // 没有新技能时增加属性
            player.attack += 5;
      ...
      if (checkAndLevelUp(player)) {
        seal.replyToSender(
          ctx,
          msg,
          `${player.name} 升级了！当前等级: ${player.level}`
        );
      }
    ...
    if (Math.random() < boss.eggChance) {
      ...
      const eggManager = managePenguinEggData(
  ...
  let bossCooldown = loadDatabase(dbKeyBossCooldown, {
    count: 0,
    lastReset: Date.now(),
  });
  const loadGroupBossData = (groupId) => {
    ...
    return data
      ? JSON.parse(data)
      : { bossState: null, boss: null, bossParticipants: {} };
  };

  // 保存数据到数据库
  const saveDatabase = (key, value) =>
    ext.storageSet(key, JSON.stringify(value));
  const saveGroupBossData = (groupId, groupBossData) => {
    ext.storageSet(
      `${dbKeyGroupBoss}_${groupId}`,
      JSON.stringify(groupBossData)
    );
  };
  ...
  const attemptInfiltrateHouse = (player, players) => {
    // 随机选取目标房主
    const allPlayers = Object.entries(players).filter(
      ([id, p]) => id !== player.userId
    ); // 排除自己
    if (allPlayers.length === 0) {
      return { success: false, message: "冒险中没有找到可侵入的房产。" };
    }

    const targetPlayerEntry =
    ...
    if (!targetHouseData || targetHouseData.level <= 0) {
      return {
        success: false,
        message: `你误入了 ${targetPlayer.name} 的一片空地，没有任何收获。`,
      };
    }
    ...
    if (player.speed > targetHouseData.security) {
      ...
      const stolenItemIndex =
        targetHouseData.storageItems?.length > 0
          ? Math.floor(Math.random() * targetHouseData.storageItems.length)
          : null;
      const stolenItem =
        stolenItemIndex !== null
          ? targetHouseData.storageItems.splice(stolenItemIndex, 1)[0]
          : null;
      ...
      return {
        ...
        message: `你成功潜入了 ${
          targetPlayer.name
        } 的房产，偷走了 ${stolenGold} 金币${
          stolenItem ? ` 和一个物品：${stolenItem.name}` : ""
        }！`,
      };
    } else {
      // 房产防御成功
      ...
      const droppedItem =
        droppedItemIndex !== null
          ? player.backpack.splice(droppedItemIndex, 1)[0]
          : null;
      ...
      return {
        ...
        message: `你被 ${
          targetPlayer.name
        } 的房产安保系统拦截，损失了 ${fineGold} 金币${
          droppedItem ? ` 和一个物品：${droppedItem.name}` : ""
        }！`,
      };
    }
  };
  ...
  const manageHouseData = (userId) => {
    ...
    const loadHouse = () => {
      ...
      return data
        ? JSON.parse(data)
        : { level: 1, income: 10, security: 1, storage: 0, lastCollectTime: 0 };
    };
...
`;
  ...
  cmdPenguinBattle.solve = async (ctx, msg, cmdArgs) => {
    ...
    if (
      !players[userId] &&
      cmdArgs.getArgN(1) !== "新建角色" &&
      cmdArgs.getArgN(1) !== "help"
    ) {
      seal.replyToSender(
        ctx,
        msg,
        '请先创建角色！使用 ".penguin 新建角色" 或查看帮助 ".penguin help"'
      );
      return;
    }
...
技能: ${player.skills
      ...
      case "状态": {
        const nextLevelExp = calculateNextLevelExp(player.level);
        const expToNextLevel = nextLevelExp - player.exp;
        const skillDescriptions = player.skills
          .map(
            (skill, index) =>
        ...
        const titleList =
          titles.length > 0 ? `称号: ${titles.join(", ")}` : "暂无称号";
        seal.replyToSender(
          ctx,
          msg,
          `
...
${titleList}`
      ...
      case "备战": {
        ...
        if (players[userId]?.resting) {
          seal.replyToSender(
            ctx,
            msg,
            '你正在休息中，无法进行其他活动！请使用 ".penguin 结束休息" 结束后再操作。'
          );
          return;
        }
        resetBossCooldown();
        ...
        seal.replyToSender(
          ctx,
          msg,
          '开始筹备 BOSS 战！请使用 ".penguin 加入" 加入战斗。'
        );
      }
      ...
      case "攻击": {
        ...
        if (
          groupBossData.bossState === "fighting" &&
          groupBossData.bossParticipants[userId]
        ) {
          ...
          if (skillArg) {
            ...
            if (result.damage)
              response += `，对目标造成了 ${result.damage} 点伤害`;
            if (result.defense)
              response += `，将自己的防御提升了 ${result.defense} 倍`;
            if (result.attack)
              response += `，使对方的攻击降低为原来的 ${result.attack} 倍`;
            if (result.frozen) response += `，并使目标进入冻结状态`;
            if (result.reflash) response += `，并使目标复活`;
            if (result.healing)
              response += `，恢复了 ${result.healing} 点生命值`;
            if (result.shield)
              response += `，为自己添加了一个可吸收 ${result.shield} 点伤害的护盾`;
            if (result.poison) response += `，并使目标进入中毒状态`;
            ...
            seal.replyToSender(
              ctx,
              msg,
              `你对 BOSS 造成了 ${damage} 点伤害！BOSS 剩余血量：${boss.health}`
            );
          }
        ...
        if (skillArg) {
          ...
          seal.replyToSender(
            ctx,
            msg,
            `你对小企鹅造成了 ${damage} 点伤害！小企鹅剩余血量：${penguin.health}`
          );
        }
      ...
      case "冒险": {
        ...
        if (eventRoll < infiltrateChance) {
          ...
          if (encounter.effectId.includes("+")) {
            ...
            if (availableSkills.length > 0) {
              ...
              seal.replyToSender(
                ctx,
                msg,
                `可惜啊年轻人，你已经打遍天下无敌手，技能书无法再为你的技能成长了！那么……\n${player.name} 增加了攻击力和生命值！`
              );
            }
          } else {
          ...
          eventRoll <
          infiltrateChance + encounterChance + bossChance
        ) {
          // BOSS 生成事件
          if (
            groupBossData.bossState !== "preparing" &&
            ...
            seal.replyToSender(
              ctx,
              msg,
              '你在冒险中触发了一个强大的 BOSS！请使用 ".penguin 加入" 加入战斗。'
            );
          } else {
            seal.replyToSender(
              ctx,
              msg,
              "当前已有正在筹备或进行中的 BOSS 战！本次冒险没有新发现。"
            );
          }
        } else {
          // 小企鹅生成事件
      ...
      case "休息": {
        ...
        if (player.resting) {
          seal.replyToSender(
            ctx,
            msg,
            '你已经在休息中，请使用 ".penguin 结束休息" 结束后再操作！'
          );
          return;
        }
      ...
      case "使用": {
        ...
        let itemIndex = parseInt(itemArg, 10) - 1;
        if (isNaN(itemIndex)) {
          itemIndex = player.backpack.findIndex(
            (item) => item.name === itemArg
          );
          if (itemIndex === -1) {
            seal.replyToSender(ctx, msg, "无效物品序号或物品名！");
            return;
        ...
        if (
          isNaN(itemIndex) ||
          itemIndex < 0 ||
          itemIndex >= player.backpack.length
        ) {
      ...
      case "转赠": {
        ...
        seal.replyToSender(
          ctx,
          msg,
          `你成功将 "${item.name}" 转赠给了 ${targetPlayer.name}！`
        );
        return;
      }
      ...
      case "孵化": {
        ...
        if (timeLeft > 0) {
          ...
          seal.replyToSender(
            ctx,
            msg,
            `你的企鹅蛋正在孵化中，还有 ${hoursLeft} 小时 ${minutesLeft} 分钟孵化完成。`
          );
        } else {
      ...
      case "互动": {
        ...
        switch (interactionType) {
          case "喂食": {
            ...
            seal.replyToSender(
              ctx,
              msg,
              `你喂食了你的宠物企鹅，亲密度增加了5点。当前亲密度: ${penguin.affection}`
            );
            break;
          }
          case "玩耍": {
            ...
            seal.replyToSender(
              ctx,
              msg,
              `你和你的宠物企鹅玩耍了一会儿，亲密度增加了3点。当前亲密度: ${penguin.affection}`
            );
            break;
          }
          case "训练": {
            ...
            seal.replyToSender(
              ctx,
              msg,
              `你训练了你的宠物企鹅，等级提升了1级。当前等级: ${penguin.level}`
            );
            break;
          }
          case "惩罚": {
            ...
            seal.replyToSender(
              ctx,
              msg,
              `你惩罚了你的宠物企鹅，亲密度减少了5点。当前亲密度: ${penguin.affection}`
            );
            break;
          }
          default: {
        ...
        if (penguin.affection <= 10) {
          ...
          seal.replyToSender(
            ctx,
            msg,
            `你的宠物企鹅离家出走了，成为了 ${newOwner.name} 的宠物企鹅！`
          );
        }
        saveDatabase(dbKeyPlayerData, players);
        return;
      ...
      case "升级房产": {
        ...
        if (player.gold < upgradeCost) {
          seal.replyToSender(
            ctx,
            msg,
            `金币不足，无法升级房产！升级需要 ${upgradeCost} 金币。`
          );
          return;
        }
      ...
      case "收取收益": {
        ...
        if (
          timeSinceLastCollect < collectInterval &&
          houseData.storage === 0 &&
          (!houseData.storageItems || houseData.storageItems.length === 0)
        ) {
          ...
          seal.replyToSender(
            ctx,
            msg,
            `距离下次收取收益还有 ${hoursLeft} 小时 ${minutesLeft} 分钟。`
          );
          return;
        }
        ...
        seal.replyToSender(
          ctx,
          msg,
          `你收取了 ${collectAmount} 金币的房产收益！${itemInfo}`
        );
        return;
      }
      ...
      case "签到": {
        ...
        if (now - lastSignIn < signInInterval) {
          ...
          seal.replyToSender(
            ctx,
            msg,
            `距离下次签到还有 ${hoursLeft} 小时 ${minutesLeft} 分钟。`
          );
          return;
        }
        ...
        seal.replyToSender(
          ctx,
          msg,
          `签到成功！你获得了 ${signInReward} 金币。当前连续签到天数: ${signInData.streak}`
        );
        return;
      }
      ...
      case "管理": {
        ...
        switch (subCommand) {
          case "奇遇": {
            const encounterList = defaultEncounters
              .map(
                (encounter, index) => `${index + 1}. ${encounter.description}`
              )
              .join("\n");
          ...
          case "移除奇遇": {
            ...
            if (
              isNaN(index) ||
              index < 0 ||
              index >= defaultEncounters.length
            ) {
            ...
            seal.replyToSender(
              ctx,
              msg,
              `成功移除奇遇事件: ${removed[0].description}`
            );
            break;
          }

          case "商店": {
            const shopList = defaultShop
              .map(
                (item, index) =>
                  `${index + 1}. ${item.name} - ${item.description} (${
                    item.cost
          ...
          case "添加商店": {
            const [name, description, cost, effectString] = cmdArgs
              .getArgN(3)
              .split("|");
...
使用@指定对象，未指定就是自身。`
              ...
              case "金币": {
                ...
                seal.replyToSender(
                  ctx,
                  msg,
                  `作弊成功！设置 ${targetPlayer.name} 的金币数量为 ${value} 。`
                );
                break;
              }
              ...
              case "经验": {
                ...
                seal.replyToSender(
                  ctx,
                  msg,
                  `作弊成功！给 ${targetPlayer.name} 增加了 ${value} 经验值。\n当前经验值: ${targetPlayer.exp}`
                );
                break;
              }
              ...
              case "等级": {
                ...
                seal.replyToSender(
                  ctx,
                  msg,
                  `作弊成功！设置 ${targetPlayer.name} 的等级为 ${value}！`
                );
                break;
              }
              ...
              case "攻击": {
                ...
                seal.replyToSender(
                  ctx,
                  msg,
                  `作弊成功！设置 ${targetPlayer.name} 的攻击力为 ${value}！`
                );
                break;
              }
              ...
              case "防御": {
                ...
                seal.replyToSender(
                  ctx,
                  msg,
                  `作弊成功！设置 ${targetPlayer.name} 的防御力为 ${value}！`
                );
                break;
              }
              ...
              case "生命": {
                ...
                seal.replyToSender(
                  ctx,
                  msg,
                  `作弊成功！设置 ${targetPlayer.name} 的生命值为 ${value}！`
                );
                break;
              }
              ...
              case "魔力": {
                ...
                seal.replyToSender(
                  ctx,
                  msg,
                  `作弊成功！设置 ${targetPlayer.name} 的魔力值为 ${value}！`
                );
                break;
              }
              ...
              case "速度": {
                ...
                seal.replyToSender(
                  ctx,
                  msg,
                  `作弊成功！设置 ${targetPlayer.name} 的速度为 ${value}！`
                );
                break;
              }
              ...
              default: {
                seal.replyToSender(
                  ctx,
                  msg,
                  '未知作弊指令！请使用 ".penguin 管理 作弊 帮助" 查看帮助。'
                );
                break;
              }
            }
          ...
          default: {
            seal.replyToSender(
              ctx,
              msg,
              "未知管理员指令，请使用 .penguin 管理 帮助 查看帮助！"
            );
            break;
          }
        }
